...
const JobBoard = () => {
  ...
  let filteredJobs = jobs.filter(job => {
    ...
    const matchesApplied = viewMode === 'all' || (viewMode === 'applied' && appliedJobs.includes(job.url));
      ...
      <div className="filters glass-panel" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <div className="input-group" style={{ flex: 2, minWidth: '250px' }}>
            <label><Search size={14} style={{ display: 'inline', marginRight: '4px', verticalAlign: 'middle' }}/> Search Keywords</label>
            <input 
              type="text" 
